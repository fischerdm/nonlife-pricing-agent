...
- **Sidebar pipeline graph** — one node per stage (Feature Selection, GBM Training, GLM Distillation, GLM Fitting), solid and filled once that stage has run, dashed while still pending, reading the same completion flags as the Audit Trail.
...
**Known gap:** none of the above yet warns when a stage's last output has gone stale relative to a *newer* upstream approval — e.g. GLM Fit not re-run after more terms are approved in Distillation. Version pickers and lineage tracing exist; automatic staleness warnings do not yet.
