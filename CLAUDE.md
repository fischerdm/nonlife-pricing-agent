...
**`dashboard/glm_workbench.py` still has none of the above** — every fix and feature in this section (checkbox defaults, actuary-owned tab placement, locked view, snapshot browser, comment history) was scoped to the feature workbench only. Apply the same patterns here when it's next touched.
...
**Next planned work:** port all of the Feature Workbench's fixes above to `dashboard/glm_workbench.py`; an `updated_at` staleness badge in both workbenches (seed configs already carry the timestamp — just not surfaced in the UI yet); origin badges (🔒 actuary / 🤖 agent / 🚫 agent-against). Explicitly deferred (raised, not started): gating the GLM stage so it can only read *finalized* feature snapshots rather than whatever's currently in `project_config.yaml` — the finalized-snapshot storage exists now, but no enforcement reads from it yet.
